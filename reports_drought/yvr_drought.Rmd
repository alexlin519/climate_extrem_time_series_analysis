...
```{r, echo=FALSE,warning=FALSE,message=FALSE}
#fliter the year
ranked_data_1941 <- ranked_data %>% filter(year == 1941)
ranked_data_1941
#2021
ranked_data_2021 <- ranked_data %>% filter(year == 2021)
ranked_data_2021
```
...
```{r, echo=FALSE,warning=FALSE,message=FALSE}
# Scatter plot with avgexc on the y-axis, station on the x-axis, and rank represented by size
ggplot(ranked_data_1941, aes(x = station, y = avgexc, size = rank, color = station)) +
  ...
  scale_size_continuous(
    ...
    range = c(10, 1),  # Adjust this range based on the desired size scale
    limits = c(1, 300)
  ) +
  labs(title = "Scatter Plot of Avgexc by Station with Rank as Size",
       x = "Station",
       y = "Avgexc",
       size = "Rank") +
  theme_minimal() +
...
#histogram
ggplot(ranked_data_1941, aes(x = avgexc, fill = station)) +
  geom_histogram(binwidth = 0.5, alpha = 0.7, position = "identity") +
  labs(title = "Histogram of Average Excess Temperature by Station",
       x = "Average Excess Temperature (°C)",
       y = "Frequency",
       fill = "Station") +
  theme_minimal() +
  theme(legend.position = "top")

#ranked_data_1941$rank <- as.numeric(as.character(ranked_data_1941$rank))
ggplot(ranked_data_1941, aes(x = month, y = avgexc, color = station, size = rank)) +
  geom_point(alpha = 0.6) +
...
ggplot(ranked_data_2021, aes(x = month, y = avgexc, color = station, size = rank)) +
  geom_point(alpha = 0.6) +
  geom_text(aes(label = rank), vjust = -0.5, size = 3, color = "black") +  # Add rank labels
  labs(title = "Average Excess Temperature Over Years by Station",
       x = "Year",
