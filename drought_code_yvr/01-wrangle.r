...
process_data <- function(file_path,station_name) {
  
  # Read the CSV file
  df <- read.csv(file_path, header = TRUE)
  
  # Rename columns and mutate data
  df <- df %>%
  ...
  imissbycol = apply(is.na(df),2,sum)
...
# 
# 
# yvr = read.csv("../output/YVR_raw_filtered_columns.csv",header=T)#,nrow=27899)
# yvr = yvr %>%
#   rename(year=LOCAL_YEAR, month=LOCAL_MONTH, date=LOCAL_DATE, 
#    mintemp=MIN_TEMPERATURE, meantemp=MEAN_TEMPERATURE, maxtemp=MAX_TEMPERATURE,
#    totprec=TOTAL_PRECIPITATION) %>%
