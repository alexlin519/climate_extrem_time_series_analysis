...
library(dplyr)
library(magrittr)
compute_avg_difference_90th <- function(file_path_temp_precip, file_path_stat,station_name) {
  # Load the required datasets
  load(file_path_temp_precip)  # yvr, maxtemp, yvr_q90
  load(file_path_stat)  # yvrstat.RData
  ...
  exc <- data.frame(avgexc = round(byMonth$avgexc, 3), nexc = byMonth$nexc,
  ...
  file_name <- paste0(station_name, "_maxtemp_avgexc_bymonth.csv")
  file_name2 <- paste0(station_name, "_maxtemp_exc_over90th.csv")
  # Create the full path to the file using file.path()
  full_file_path <- file.path(save_path, file_name)
  full_file_path2 <- file.path(save_path, file_name2)
  
  # Save the data and missing information
  write.csv(file = full_file_path, exc, row.names = FALSE)
  write.csv(file = full_file_path2, diff, row.names = FALSE)
