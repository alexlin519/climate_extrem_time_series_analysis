...
process_heat_exceedances <- function(temp_precip_path, stat_path, station_name) {
  ...
  if (iex) { end_ex = c(end_ex, iend); excvec = c(excvec, ss / len) }
  periods_ex = cbind(begin_ex, end_ex, excvec)
  ...
  heatwave_csv_path = paste0("../drought_code_yvr/data/", station_name, "_dates_heatwave2.csv")
  write.csv(file = heatwave_csv_path, dates_heatwave, row.names = FALSE)
  
  return(heatwave_csv_path)
}



# # Exceedances of 90% in heat periods with 3 or more consecutive exceedances
# # This file can replace 03-heatdays.r
# 
...
# #----------------------------------------------------------------------
# 
# # Code is now different from 03-heatdays.r
# 
# # Find periods with three of more consecutive exceedances
# # Add the amount of exceedance
# n = length(iexceed)
# begin_ex = NULL; end_ex = NULL
# iex = iexceed[1]
# excvec = NULL; ss = 0; len = 0;
# if(iex) { begin_ex = c(begin_ex,1); iend = i }
# #for(i in 2:1000)  # for testing
# for(i in 2:n)
# { if(iex & iexceed[i]) { iend = i; ss = ss+exc[i]; len = len+1; }
#   else if(iex & !(iexceed[i])) 
#   { end_ex = c(end_ex,iend); iex = F; excvec = c(excvec,ss/len); ss =0; len=0 }
#   else if(!iex & iexceed[i]) 
#   { begin_ex = c(begin_ex,i); iend = i; iex = T; ss = ss+exc[i]; len = len+1; }
...
#   endtemp=maxtemp[periods_heat[,2]], avgexc = round(periods_heat[,3],3))
# 
# #----------------------------------------------------------------------
# 
...
# dates_heatwave$month = ymd_heat[,2]
# 
# #sink("yvr_dates_heatwave2.txt")
# #print(dates_heatwave)
# # 327 cases (average of 327/87 = 3.8 per year)
# #sink()
# write.csv(file="yvr_dates_heatwave2.csv", dates_heatwave, row.names=F)
# 
