...
rank_colors <- c("1" = "red", "2" = "orange", "3" = "yellow", "4" = "green", "5" = "blue", "6" = "purple")
...
ehf_1y_add_text_annotation <- function(ehf_potato, start_year, end_year, year_of_interest,station_name,
                                       data_pot, top_yields) {
  ...
  plot_ehf_all(ehf_potato, start_year, end_year, year_of_interest,station_name)+ theme(plot.margin = margin(20, 20, 20, 20)) +theme(aspect.ratio = 1/2) +
  geom_vline(xintercept = 120, linetype = "dashed", color = "green")+ annotate("text", x = 200, y = 100, label = annotation_text, color = "red", size = 5, hjust = 0, vjust = 0)
}
...
#### ---eg ----
# plot_ehf_all(ehf_potato, start_year, end_year, 1995,station_name) 
# + annotate("text", x = 200, y = 100, 
#            label =paste( "Detrended_Yield:", data_pot %>% filter(REF_DATE == 1995) %>% pull(Detrended_Yield_LOESS)), color = "red", size = 5, hjust = 0, vjust = 0)
