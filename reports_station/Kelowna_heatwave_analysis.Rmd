...
plot_ehf_extreme_yield(ehf_extreme_yield,start_year, end_year, "Kelowna",  (1:6),  top_n = 2)

plot_ehf_extreme_yield(ehf_extreme_yield,start_year, end_year, "Kelowna",  (1:6),  lowest_n = 2)
...
plot_ehf_extreme_yield <- function(ehf_extreme_yield, start_year, end_year, station_name, ranking, top_n = NULL, lowest_n = NULL) {
  ...
  ggplot(all_years_data, aes(x = DayOfYear)) +
    geom_line(aes(y = EHF, color = Color, group = interaction(Year, Rank)), size = 0.5, alpha = 0.4) +
    scale_color_manual(values = c("highest yield" = "red", "lowest yield" = "blue"), name = "Yield Type") +
    labs(
      title = paste("EHF Over the Years with Top and Lowest", las_dig ," Yields at", station_name),
      x = "Day of Year",
      y = "EHF"
    ) +
    theme_minimal() +
...
```{r, echo=FALSE,message=FALSE, warning=FALSE}
ehf_1y_add_text_annotation(ehf_potato, start_year, end_year, 1941,station_name,
                           data_pot, top_yields)
